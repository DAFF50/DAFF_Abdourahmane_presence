...
@section('content')
    ...
    @endif
    <a class="btn btn-primary" href="{{ route('addServices') }}">
        Nouveau
    ...
    <table class="table mt-4">
        ...
        @foreach($services as $s)
            <tr>
                <td>{{$s->id}}</td>
                <td>{{$s->libelle}}</td>
                <td>{{$s->departement->libelle}}</td>
                <td>
                    <div class="modal" tabindex="-1" id="deletemodal{{$s->id}}">
                        <form action="{{route("deleteServices",$s->id)}}" method="post">
                            @csrf
                            @method('delete')
                    ...
                    <a class="btn btn-primary" href="{{route('editServices',$s->id)}}">
                        Modifier
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
